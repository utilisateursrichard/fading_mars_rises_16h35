...
export const BookmarkView: React.FC = () => {
  ...
  const bookmarkletCode = `javascript:(function(){const OVERLAY_ID='betterschool-overlay';const existing=document.getElementById(OVERLAY_ID);if(existing){existing.remove();return;}const overlay=document.createElement('div');overlay.id=OVERLAY_ID;overlay.style.cssText='position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:2147483647;border:none;background:#F8FAFC;margin:0;padding:0;overflow:hidden;';const iframe=document.createElement('iframe');iframe.src='${effectiveOrigin}';iframe.style.cssText='width:100%;height:100%;border:none;display:block;';iframe.title='BetterSchool Interface';const closeBtn=document.createElement('button');closeBtn.textContent='✕ Quitter BetterSchool';closeBtn.style.cssText='position:fixed;bottom:20px;right:20px;z-index:2147483648;background:#0f172a;color:#fff;border:1px solid rgba(255,255,255,0.15);padding:8px 16px;border-radius:9999px;font-family:sans-serif;font-size:13px;font-weight:600;cursor:pointer;box-shadow:0 10px 25px -5px rgba(0,0,0,0.3);transition:all 0.2s;';closeBtn.onmouseenter=()=>closeBtn.style.transform='scale(1.05)';closeBtn.onmouseleave=()=>closeBtn.style.transform='scale(1)';closeBtn.onclick=()=>overlay.remove();const handleEsc=(e)=>{if(e.key==='Escape'){overlay.remove();window.removeEventListener('keydown',handleEsc);}};window.addEventListener('keydown',handleEsc);overlay.appendChild(iframe);overlay.appendChild(closeBtn);document.body.appendChild(overlay);})();`;
